...
function postDiagnostic(message, level = 'loading') {
...
figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (msg.type === 'scrape-result') {
        ...
        console.log('[Plugin][scrape-result] Received UI scrape result — trackId:', trackId, '| iPhone:', (_b = (_a = msg.iphone) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0, '| iPad:', (_d = (_c = msg.ipad) === null || _c === void 0 ? void 0 : _c.length) !== null && _d !== void 0 ? _d : 0);
        pending.resolve({
    ...
    if (msg.type === 'search-app') {
        ...
        try {
            const results = yield searchApp(msg.appName, msg.country);
    ...
    if (msg.type === 'load-rank') {
        try {
            const results = yield loadRankData(msg.rank, msg.country, msg.genre, msg.appName, msg.trackId);
            figma.ui.postMessage({
                type: 'rank-results',
                apps: results
            });
            ...
            for (let i = 0; i < apps.length; i++) {
                ...
                if ((!appToImport.screenshotUrls || appToImport.screenshotUrls.length === 0) && appToImport.trackId) {
                    ...
                    if (data.results && data.results.length > 0) {
                        ...
                        console.log(`[Plugin][batch-import] Details fetched — screenshotUrls: ${(_f = (_e = appToImport.screenshotUrls) === null || _e === void 0 ? void 0 : _e.length) !== null && _f !== void 0 ? _f : 0}, ipadScreenshotUrls: ${(_h = (_g = appToImport.ipadScreenshotUrls) === null || _g === void 0 ? void 0 : _g.length) !== null && _h !== void 0 ? _h : 0}`);
                    }
                    else {
                    ...
                    console.log(`[Plugin][batch-import] App already has screenshotUrls: ${(_k = (_j = appToImport.screenshotUrls) === null || _j === void 0 ? void 0 : _j.length) !== null && _k !== void 0 ? _k : 0}`);
                }
                const result = yield importScreenshots(appToImport, msg.device, msg.country, currentY);
    ...
    if (msg.type === 'import-screenshots') {
        console.log('[Plugin][import-screenshots] Start — app:', (_l = msg.app) === null || _l === void 0 ? void 0 : _l.trackName, '| device:', msg.device, '| country:', msg.country);
        try {
            ...
            if ((!appToImport.screenshotUrls || appToImport.screenshotUrls.length === 0) && appToImport.trackId) {
                ...
                if (data.results && data.results.length > 0) {
                    appToImport = data.results[0];
                    console.log('[Plugin][import-screenshots] Details fetched — screenshotUrls:', (_o = (_m = appToImport.screenshotUrls) === null || _m === void 0 ? void 0 : _m.length) !== null && _o !== void 0 ? _o : 0);
                }
                else {
                    console.warn('[Plugin][import-screenshots] No results returned from details API');
                }
            }
            const result = yield importScreenshots(appToImport, msg.device, msg.country);
 ...
 */
function searchApp(appName, country) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (allApps.size < 10) {
            ...
            try {
                ...
                const rssResponse = yield fetch(rssUrl);
                const rssData = yield rssResponse.json();
                if ((_a = rssData.feed) === null || _a === void 0 ? void 0 : _a.results) {
                    const appIds = rssData.feed.results
                        .map((r) => r.id)
                        .filter((id) => !allApps.has(id) && (!trackId || id !== String(trackId)))
                        .slice(0, 20);
                    if (appIds.length > 0) {
                        const batchUrl = `https://itunes.apple.com/lookup?id=${appIds.join(',')}&country=${country}`;
                        const batchResponse = yield fetch(batchUrl);
...
function scrapeAppStoreScreenshots(trackId, country) {
    return __awaiter(this, void 0, void 0, function* () {
 ...
 */
function importScreenshots(app, device, country, startY) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((!app.screenshotUrls || app.screenshotUrls.length === 0) && app.trackId) {
            ...
            if (data.results && data.results.length > 0) {
                ...
                console.log('[importScreenshots] Details received — screenshotUrls:', (_b = (_a = appDetails.screenshotUrls) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0, '| ipadScreenshotUrls:', (_d = (_c = appDetails.ipadScreenshotUrls) === null || _c === void 0 ? void 0 : _c.length) !== null && _d !== void 0 ? _d : 0, '| genres:', appDetails.genres);
            }
            else {
            ...
            console.log('[importScreenshots] App already has details — screenshotUrls:', (_f = (_e = app.screenshotUrls) === null || _e === void 0 ? void 0 : _e.length) !== null && _f !== void 0 ? _f : 0);
        }
        ...
        if (noScreenshots) {
            ...
            if (appDetails.trackId) {
                const scraped = yield scrapeAppStoreScreenshots(String(appDetails.trackId), country);
                if (scraped.iphone.length > 0 || scraped.ipad.length > 0) {
                    ...
                    appDetails = Object.assign(Object.assign({}, appDetails), { screenshotUrls: scraped.iphone.length > 0 ? scraped.iphone : scraped.ipad, ipadScreenshotUrls: scraped.ipad });
                }
                else {
        ...
        if ((device === 'iphone' || device === 'all') && !hasIphoneScreenshots && hasIpadScreenshots) {
            ...
            appDetails = Object.assign(Object.assign({}, appDetails), { screenshotUrls: appDetails.ipadScreenshotUrls });
        }
        ...
        if (appDetails.artworkUrl512 || appDetails.artworkUrl100) {
            ...
            const iconNode = yield createImageNode(iconUrl, 'App Icon');
            if (iconNode) {
                iconNode.x = 32;
                iconNode.y = yOffset;
        ...
        if ((device === 'all' || device === 'iphone') && appDetails.screenshotUrls && appDetails.screenshotUrls.length > 0) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const imageNode = yield createImageNode(screenshotUrl, `iPhone Screenshot ${i + 1}`);
                if (imageNode) {
                    imageNode.x = xOffset;
                    imageNode.y = yOffset;
        ...
        if ((device === 'all' || device === 'ipad') && appDetails.ipadScreenshotUrls && appDetails.ipadScreenshotUrls.length > 0) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const imageNode = yield createImageNode(screenshotUrl, `iPad Screenshot ${i + 1}`);
                if (imageNode) {
                    imageNode.x = xOffset;
                    imageNode.y = yOffset;
 ...
 */
function createImageNode(url, name) {
    return __awaiter(this, void 0, void 0, function* () {
